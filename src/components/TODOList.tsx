...
export function TODOList({ todos, setTodos }: { todos: Todo[]; setTodos: SetTodos }) {
    return (
        <ol className="todo-list">
            {todos && todos.length > 0 ? (
                todos.map((item: Todo, index: number) => <Item key={index} item={item} todos={todos} setTodos={setTodos} />)
            ) : (
...
export function Item({ item, setTodos, todos }: { item: Todo; setTodos: SetTodos; todos: Todo[] }) {
    const completeTodo = () => {
    ...
    const handleDelete = () => {
        setTodos((prevTodos: Todo[]) => prevTodos.filter((todo: Todo) => todo.id !== item.id));
